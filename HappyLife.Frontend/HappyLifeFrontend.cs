...
namespace HappyLife.Frontend
{
    ...
    public class HappyLifeFrontend : TaiwuRemakeHarmonyPlugin
    {
        ...
        {
            //File.AppendAllText("HappylifeDebug.log", $"{logContent}\r\n");
        }
                        ...
                        if (lines.Length != 0)
                        {
                            ...
                            for (var lineIndex = 1; lineIndex < lines.Length; lineIndex++)
                            {
                                ...
                                if (id >= 1 && id <= 20)
                                {
                                    ...
                                    typeof(BuildingBlockItem).GetField(nameof(BuildingBlockItem.Class), BindingFlags.Instance | BindingFlags.Public).SetValue(originalBlockItem, EBuildingBlockClass.BornResource);
                                ...
                                {
                                    Log($"Remove dependings for {originalBlockItem.Name}");
            ...
            public static void Postfix(UI_BuildingOverview __instance)
            {
                if (GetBoolSettings("EnableBuildResource"))
                {
                    ...
                    BuildingBlock.Instance.Iterate(delegate (BuildingBlockItem item)
                    {
                        ...
                        {
                            if (!_buildingMap.TryGetValue(EBuildingBlockClass.BornResource, out var value))
                            {
                                value = new List<BuildingBlockItem>();
                                _buildingMap.Add(EBuildingBlockClass.BornResource, value);
                            }
                            // Skip useless buildings
                            if (item.TemplateId != 21 && item.TemplateId != 22 && item.TemplateId != 23)
                            {
                                if (value.Any(i => i.TemplateId == item.TemplateId))
                                    return true;
                                value.Add(item);
                            }
                        }
